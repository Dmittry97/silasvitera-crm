...
export default function ProductDetail() {
  ...
  const confirmAddToCart = () => {
    ...
    const item = {
      ...
      title: product.title,
      price: product.price,
      image: product.mainImage ? `/silasvitera/api/photos/${product.mainImage}` : undefined,
      size: selectedSize,
      quantity: 1,
    };
  ...
  if (product.mainImage) allImages.push(`/silasvitera/api/photos/${product.mainImage}`);
  if (product.backImage) allImages.push(`/silasvitera/api/photos/${product.backImage}`);
  if (product.otherImages) allImages.push(...product.otherImages.map(img => `/silasvitera/api/photos/${img}`));
